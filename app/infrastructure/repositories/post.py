...
class SQLPostRepository(PostRepository):
    ...
    async def get_by_user(self, user_id: int, skip: int = 0, limit: int = 100) -> List[Post]:
        """Get posts by user."""
        stmt = select(Post).where(Post.user_id == user_id).offset(skip).limit(limit).order_by(Post.created_at.desc())
        result = await self.session.execute(stmt)
        return list(result.scalars().all())
    
    async def get_by_status(self, status: PostStatus, skip: int = 0, limit: int = 100) -> List[Post]:
        """Get posts by status."""
        stmt = select(Post).where(Post.status == status).offset(skip).limit(limit).order_by(Post.created_at.desc())
        result = await self.session.execute(stmt)
        return list(result.scalars().all())
    ...
    async def archive(self, post_id: int) -> Optional[Post]:
        """Archive post."""
        stmt = update(Post).where(Post.id == post_id).values(
    ...
    async def get_by_tags(self, tags: List[str], skip: int = 0, limit: int = 100) -> List[Post]:
        """Get posts by tags."""
