...
from app.domain.entities.user import User, UserCreate, UserUpdate, UserRole
...
class UserUseCases:
    ...
    async def create_user(self, user_data: UserCreate, admin_user_id: Optional[int] = None) -> User:
        """Create a new user."""
    ...
    async def get_user_by_id(self, user_id: int, requester_id: int) -> Optional[User]:
    ...
    async def get_all_users(self, requester_id: int, skip: int = 0, limit: int = 100) -> List[User]:
    ...
    async def update_user(self, user_id: int, user_data: UserUpdate, requester_id: int) -> Optional[User]:
    ...
    async def get_users_by_role(self, role: UserRole, requester_id: int, skip: int = 0, limit: int = 100) -> List[User]:
    ...
    async def search_users(self, query: str, requester_id: int, skip: int = 0, limit: int = 100) -> List[User]:
        ...
        all_users = await self.user_repository.get_all(0, 1000)  # Get more users for search
        
        query_lower = query.lower()
        filtered_users = [
            user for user in all_users
            if (query_lower in user.first_name.lower() or
                query_lower in user.last_name.lower() or
